...
function showSection(sectionId, btn, skipHistory) {
    ...
    if (btn) {
        ...
        const matchingBtn = document.querySelector(`.nav-btn[onclick*="'${sectionId}'"]`);
...
function handleHash() {
    ...
    if (validSections.includes(hash)) {
        ...
        if (defaultBtn) {
            const match = defaultBtn.getAttribute('onclick').match(/showSection\('([^']+)'/);
...
function loadChecklist() {
    ...
    if (saved) {
        ...
        const items = document.querySelectorAll('.checklist-item');
        items.forEach((item, i) => { if (state[i]) item.classList.add('checked'); });
    }
}
...
function updateCountdown() {
    ...
    if (now >= tripStart && now <= tripEnd) {
        ...
        `;
        if (statusEl) {
            statusEl.innerHTML = '✈️ Die Reise läuft – noch ' + days + ' Tag' + (days !== 1 ? 'e' : '') + ' unterwegs';
            statusEl.className = 'trip-status';
            statusEl.style.display = 'block';
        }
    } else if (now > tripEnd) {
        // Trip ended
        countdownEl.innerHTML = '<p style="color: var(--primary-accent); font-family: Playfair Display, serif; font-size: 1.3rem; font-style: italic;">Bis zum nächsten Abenteuer!</p>';
        if (statusEl) {
